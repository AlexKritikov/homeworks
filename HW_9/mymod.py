from Export_file import fibonacci
import sys

from pprint import pprint

# Task 1. Imports practice
# Make a directory with 2 modules; make a function in one of them; then import this function in the other module and use
# that in your script of choice.
print(f'If we need the fifth Fibonacci number we can use our import function and get {fibonacci(5)}')

# Task 2. The sys module.
# The “sys.path” list is initialized from the PYTHONPATH environment variable. Is it possible to change it from
# within Python? If so, does it affect where Python looks for module files? Run some interactive tests to find it out.

sys.path.append('\\Users\\alexkritikov\\Desktop\\Питон')
from Train import tree
print(f'If we want to look nice Christmas Tree 5 levels we can use our import function from desktop directory and get')
tree(5)

# Task 3. Basics, import, work with os module#
# Write a program that counts lines and characters in a file (similar to `wc` Unix-utility, for additional info about it
# follow the link: https://www.geeksforgeeks.org/wc-command-linux-examples/ or in case you have macOS or
# Linux - just call manual for this utility via command: `man wc`).#  Create a Python module called "mymod.py",
# which has three functions:#

# count_lines(name) function that reads an input file and counts the number of lines in it (hint: file.readlines()
# does most of the work for you, and "len" does the rest)
# count_chars(name) function that reads an input file and counts the number of characters in it (hint: file.read()
# returns a single string)
# test(name) function that calls both counting functions with a given input filename. Such a filename generally
# might be passed-in, hard-coded, input with input(), or pulled from a command-line via the sys.argv list; for now,
# assume it’s a passed-in function argument.

# All three `mymod.py` functions should expect a filename string to be passed in.
# Test your module interactively, using import and name qualification to fetch your exports.
# # Does your PYTHONPATH need to include the directory where you created mymod.py?
# # Try running your module on itself: e.g., test("mymod.py"). Note that the test opens the file twice;
# if you’re feeling ambitious, you may be able to improve this by passing an open file object into the two count
# functions (hint: file.seek(0) is a file rewind).

# 'C:\\Users\\alexkritikov\\Desktop\\Treasure_Island.txt'
def count_lines(name):
    return (len(open(name,'r').readlines()))


def count_chars(name):
    chars=0
    for i in open(name,'r').read():
        chars+=1
    return(chars)

def test(name):
    return (f'Our text consists of {count_lines(name)} lines and {count_chars(name)} characters')

# PYTHONPATH should include directory where imported module stored
# Function test('mymod.py') calculates lines (55) and symbols (2 857) in module 'mymod.py'
